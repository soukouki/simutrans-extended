		...
		if (file->is_loading()) {
			...
			if (env_t::networkmode   &&  !env_t::server) {
				...
				if (env_t::default_settings.get_with_private_paks()) {
					scenario_path = ( std::string("addons/") + env_t::objfilename + "scenario" + PATH_SEPARATOR + scenario_name.c_str() + "/").c_str();
				...
				if (rdwr_error) {
					scenario_path = (env_t::objfilename + "scenario/" + scenario_name.c_str() + PATH_SEPARATOR).c_str();
