...
void main_view_t::display(bool force_dirty)
{
	...
	case MENU_TOP:
		...
		break;
	case MENU_BOTTOM:
		clip_rr = scr_rect(0, win_get_statusbar_height(), disp_width, disp_height - env_t::iconsize.h + 1);
		break;
	case MENU_LEFT:
...
#else
	clear_all_poly_clip();
	display_region( koord( clip_rr.x, clip_rr.y ), koord( clip_rr.w, clip_rr.h - clip_rr.y ), y_min, dpy_height + 4 * 4, false );
#endif
