...
const char *terraformer_t::can_raise_all(const player_t *player, bool allow_deep_water, bool keep_water) const
{
	assert(op == terraformer_t::raise);
	assert(ready);

	for(node_t const& i : list) {
		if (const char *err = can_raise_tile_to(i, player, allow_deep_water, keep_water)) {
			return err;
		}
	}
	return NULL;
...
const char *terraformer_t::can_lower_all(const player_t *player, bool allow_deep_water) const
{
	assert(op == terraformer_t::lower);
	assert(ready);

	for(node_t const& i : list) {
		if (const char *err = can_lower_tile_to(i, player, allow_deep_water)) {
			return err;
		}
	}
...
const char *terraformer_t::can_raise_tile_to(const node_t &node, const player_t *player, bool allow_deep_water, bool keep_water) const
{
	assert(welt->is_within_limits(node.x,node.y));
	...
	const sint8 min_hgt = min(min(node.hsw,node.hse),min(node.hne,node.hnw));

	if(  gr->is_water()  &&  keep_water  &&  max_hgt > water_hgt  ) {
		return "";
	}

	if(gr->is_water() && min_hgt < welt->get_groundwater() && !allow_deep_water)
	{
...
const char* terraformer_t::can_lower_tile_to(const node_t &node, const player_t *player, bool allow_deep_water) const
{
	assert(welt->is_within_limits(node.x,node.y));
	...
	if(hneu < welt->get_groundwater() && !allow_deep_water)
	{
		return "Cannot terraform in deep water";
	}
