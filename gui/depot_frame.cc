...
bool depot_frame_t::infowin_event(const event_t *ev)
{
	...
	if(0) {
		if(IS_LEFTCLICK(ev)  ) {
			if(  !convoy_selector.getroffen(ev->click_pos.x, ev->click_pos.y-D_TITLEBAR_HEIGHT)  &&  convoy_selector.is_dropped()  ) {
				// close combo box; we must do it ourselves, since the box does not receive outside events ...
				convoy_selector.close_box();
			}
			if(  line_selector.is_dropped()  &&  !line_selector.getroffen(ev->click_pos.x, ev->click_pos.y-D_TITLEBAR_HEIGHT)  ) {
				// close combo box; we must do it ourselves, since the box does not receive outside events ...
